...
void AddVertices::createVertices()
{
	...
	{
		htr::Point3D direction(point.x - originalPoints_centroid.x, point.y - originalPoints_centroid.y, point.z - originalPoints_centroid.z);
		float magnitude = sqrtf(direction.x*direction.x + direction.y*direction.y + direction.z*direction.z);
		float points_per_segment = magnitude / distance_between_points;

		htr::Point3D normalized_direction = direction / magnitude;

		for (float i = 1; i < points_per_segment; i++)
		{
			htr::Point3D scaled_direction = normalized_direction * i * distance_between_points;
			htr::Point3D aux = originalPoints_centroid + scaled_direction;
			float distance_centroid_newPoint = sqrtf(pow(aux.x - originalPoints_centroid.x, 2) +
				+pow(aux.y - originalPoints_centroid.y, 2)+
				(pow(aux.z - originalPoints_centroid.z, 2)));
			if (distance_centroid_newPoint < magnitude)
			if (aux.x != originalPoints_centroid.x || aux.y != originalPoints_centroid.y || aux.z != originalPoints_centroid.z)
		...
		//{
		//	htr::Point3D scaled_direction = normalized_direction * i * distance_between_points;
		//	htr::Point3D aux = point + scaled_direction;
		//	aux.type = htr::Point3D::point_type::air;
		//	added_points.push_back(aux);
		//}
	}
...
void AddVertices::convertToPointCloud()
{
	pcl_point_cloud.clear();
	for (auto point : all_points)
		pcl_point_cloud.push_back(pcl::PointXYZ(point.x, point.y, point.z));
