...
__device__ float calculateSpeedVector_shared(int global_index, int shared_index, float *f, float *velocityVector_d)
{
	...
	if (ro == ro && ro != 0.0)
	{
		velocityVector_d[global_index * 3] = rovx / ro;
		velocityVector_d[global_index * 3 + 1] = rovy / ro;
		velocityVector_d[global_index * 3 + 2] = rovz / ro;
	}
	else
	{
		printf("ERROR!\n");
		velocityVector_d[global_index * 3] = 0;
		velocityVector_d[global_index * 3 + 1] = 0;
		velocityVector_d[global_index * 3 + 2] = 0;
	}
...
__global__ void stream_collide_device(float *f, unsigned int *solid, float tau, float c, float *velocityVector_d)
{
	int		iSolid, iBase, iAdvected;
	int		newI, newJ, newK;
	...
	__shared__ float f_shared[100*19];
	...
	while (k < dims[2])
	{
		...
		if (solid[iSolid] == 0)
		{
			for (int l = 0; l < 19; l++)
			{
				...
				iAdvected = I3D_S(dims[0], dims[1], dims[3], newI, newJ, newK, l);

				f_shared[k * dims[3] + l] = f[iAdvected];
			}

			//Collision phase
			ro = calculateSpeedVector_shared(iSolid, k, f_shared, velocityVector_d);
			calculateEquilibriumFunction_device(iSolid, feq, ro, c, velocityVector_d);

			for (int l = 0; l < 19; l++)
			{
				iBase = iSolid * dims[3] + l;
				f[iBase] = f_shared[k*dims[3] + l] - (f_shared[k*dims[3] + l] - feq[l]) / tau;
			}
		}
		else
			solid_BC_device(iSolid, f);
		
		k += blockDim.x;
	}
}

void latticed3q19::stream_collide(void)
{
	cudaEvent_t start, stop;
	float time;
	cudaEventCreate(&start);
	...
	stream_collide_device <<<blocks, threads >>>(f_d, solid_d, _tau, c, velocityVector_d);
