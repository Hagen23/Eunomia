			...
			if (current_cell->type != cellType::CT_OBSTACLE && current_cell->type != cellType::CT_EMPTY)
			{
				if (current_cell->type == cellType::CT_FLUID)
				{
					for (int i = 0; i < 9; i++)
					{
						...
						{
							current_cell->mass_temp += nb_cell->f[i] - current_cell->f[invVel2D[i]];
				...
				else if (current_cell->type == cellType::CT_INTERFACE)
				{
					...
					for (int i = 0; i < 9; i++)
					{
						...
						else if (nb_cell->type == cellType::CT_INTERFACE)
						{
							...
							current_cell->mass_temp += (nb_cell->f[i] - current_cell->f[invVel2D[i]])*0.5f * (current_epsilon + nb_epsilon);
...
current_cell->mass_temp += nb_cell->f[l] - current_cell->f[invVel2D[l]];
//current_cell->mass_temp += nb_cell->f[invVel2D[l]] - current_cell->f[l];
...
current_cell->mass_temp += (nb_cell->f[l] - current_cell->f[invVel2D[l]])*0.5f * (current_epsilon + nb_epsilon);
		...
		for (int col = 0; col < width - 1; col++)
		{
			...
			if (current_cell->type != cellType::CT_OBSTACLE && current_cell->type != cellType::CT_EMPTY)
			{
				...
				for (int i = 0; i < 9; i++)
				{
					...
					//To include gravity
					current_cell->f[i] += current_cell->rho * weights2D[i] * dot(vel2Dv[i], float2{ latticeAcceleration, 0 });
				}
...
//	//To include gravity
current_cell->f[l] += current_cell->rho * weights2D[l] * dot(vel2Dv[l], float2{ 0, -latticeAcceleration });
}
	...
	for (int row = 0; row < height - 1; row++)
	{
		for (int col = 0; col < width - 1; col++)
		{
			...
			else if (numIF > 0)
			{
				...
				for (int i = 0; i < 9; i++)
					current_cell->ftemp[i] = isIF[i] ? excess_mass_uniform : 0.f;
			}
		}
	}
#pragma endregion
